...
- [x] T3.9 仓储扩展：files 全量库检索（类型/群组/关键词）
- [ ] M3 验收：管理员手机上完成一次完整采集入库 + 回收/清理闭环

## M4：应用锁、打磨与发布

- [x] T4.1 应用锁：local_auth 指纹 + 图案 fallback + LockGate 拦截冷启动/后台恢复
- [x] T4.2 设置页补全：主题切换、缓存清理、服务器重配、管理模式、关于/版本
- [x] T4.3 深色细节与动效打磨、空状态/骨架屏统一、横竖屏适配
- [x] T4.4 单元测试：规则解析、级别可见性、上传队列状态机；关键 Widget 测试
- [x] T4.5 图标与应用名替换（占位图标 → 正式资源；`PicWall / 图墙`）
- [x] T4.6 发布签名：keystore secrets 接入 release 构建（或先 deliver debug 包）
