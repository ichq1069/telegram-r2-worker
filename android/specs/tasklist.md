...
- [ ] T2.5 自动同步设置：指定相册 + 开关 + 权限引导（通知/相册）
- [ ] T2.6 同步引擎：前台服务 + workmanager 增量扫描 + `sync_state` 游标 + 去重上传 + 失败待重试
- [ ] T2.7 同步状态 UI：上次同步时间、今日上传、暂停/恢复、清理待传
- [ ] M2 验收：相册新增照片自动/手动可入库；WiFi 限制生效；队列断点续传

## M3：管理端（统计 / 采集 / 回收站 / 仓储）

- [ ] T3.1 管理模式开关：admin API_KEY 校验 + AdminShell 导航（复用现有用户会话鉴权约定）
- [ ] T3.2 统计概览：`/admin/api/stats` 卡片 + `/admin/api/r2-usage`、`worker-usage` 趋势
- [ ] T3.3 采集-输入与解析：网页/链接输入 → `/admin/api/scrape/analyze`，携带 Cookie 与规则
- [ ] T3.4 采集-规则面板：忽略关键词/忽略格式/单张上限/必带内容 + 域名规则组（拉取/保存云端组）
- [ ] T3.5 采集-候选瀑布：缩略图、默认全选、全选/全不选/失败项勾选
- [ ] T3.6 采集-入库：逐张 grab_one + 进度/失败原因（已存在/忽略/超限/网络）+ 续传
- [ ] T3.7 回收站：trash 列表 + 恢复/彻底删除；未入库 unsaved 列表 + 重试
