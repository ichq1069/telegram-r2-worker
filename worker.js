...
async function groupItems(env, groupStr, limit) {
  ...
  if (ids.length) {
    ...
    const d = await stmt.bind.apply(stmt, ids).all();
    const map = {};
...
function handleDocs() {
  ...
  const note = '<div class="nb"><h3>Custom Bot API</h3><p>This Worker also provides a Telegram Bot API proxy.</p><p>Endpoint: <code>/bot/{method}</code> (POST, body = Telegram Bot API params)</p><p>Supported: sendMessage, sendPhoto, sendDocument, sendVideo, getFile, getMe, setWebhook, getUpdates, getChat, banChatMember, deleteMessage, forwardMessage, copyMessage</p><p>Example: <code>POST /bot/sendMessage</code> with body {"chat_id":"...","text":"Hello"}</p></div>';
  const eps = [
